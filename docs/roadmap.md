...
현재 프로젝트는 **가변 공격과 가변 피해를 포함하는 기본 전투 확률 계산 MVP, 선택형 웹 UI, 대표 회귀·불변식 테스트와 재현 가능한 의존성 설치가 구현된 단계**다.
...
의존성 설치는 저장소 루트의 lockfile 하나를 기준으로 한다.

- 루트 `package-lock.json`, lockfile v3
- `.npmrc`에서 lockfile 생성과 정확한 버전 저장 강제
- CI와 GitHub Pages 배포에서 `npm ci` 사용
- `actions/setup-node`의 npm 캐시가 `package-lock.json`을 키로 사용
- 임시 PR에서 clean install, 타입 검사, 테스트와 빌드 통과 확인
...
- npm Workspaces 전체를 루트 `package-lock.json` 하나로 잠금
- 워크스페이스별 lockfile 금지
- lockfile v3 사용
- 새 의존성은 정확한 버전으로 저장
- 일반 개발 설치와 의존성 변경은 저장소 루트의 `npm install` 사용
- CI와 배포는 lockfile을 수정하지 않는 `npm ci` 사용
- npm 캐시 키는 `package-lock.json`을 기준으로 설정
- `package.json`과 lockfile 변경을 같은 커밋에 포함

검증:

- GitHub Actions runner에서 `npm ci` clean install 성공
- TypeScript 검사 성공
- 단위·골든·불변식 테스트 성공
- Vite 프로덕션 빌드 성공
...
목표: 가변 주사위와 특수 규칙을 확장할 수 있도록 계산 코어를 일반화하고 재현 가능한 검증·설치 기반을 확정한다.
...
- [x] 의존성 lockfile 정책 확정

### 완료 조건

- [x] 기존 고정값 테스트 결과가 회귀하지 않는다.
- [x] 모든 공개 분포의 확률 합이 허용 오차 내에서 1이다.
- [x] D3와 D6 공격·피해의 대표 사례가 골든 테스트를 통과한다.
- [x] 재현 가능한 의존성 설치 정책을 문서화하고 CI에 적용한다.
- [x] `npm ci` 이후 `npm run check`가 통과한다.
...
다음 개발 작업은 **정식 데이터 스키마를 정의하고 현재 TypeScript 샘플 카탈로그를 외부 JSON으로 이전하는 것**이다.
