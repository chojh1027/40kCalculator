...
현재 웹 카탈로그는 자체 작성 샘플 데이터입니다. Intercessor Squad의 임시 Sustained Hits와 Lethal Hits 무장은 정식 `Ability` 레코드와 `effects` 필드를 통해 계산 규칙에 연결됩니다.
...
다음 작업은 **버전 manifest, 진영별 데이터 청크와 IndexedDB 기반 로컬 데이터 저장 구조를 설계하고 구현하는 것**입니다.
