...
기본 전투 계산 MVP와 선택형 UI가 구현되어 있으며, 가변 주사위를 위한 확률 계산 코어를 확장하고 있습니다.
...
- 공격 횟수별 명중·상처·내성·최종 상태 분포의 PMF 합성
- UI에서 공격 주사위 표현과 공격 횟수 분포 표시
- 임시 `D6 Test Blaster (Temporary)` 샘플 무장
- 고정 피해 계산
- 일반 피해의 초과 피해 비전달(non-spill) 처리
- 다중 운드 모델의 피해 할당
- 평균 유효 피해, 평균 파괴 모델 수, 최빈 결과, 전멸 확률
- 공격 횟수를 포함한 단계별 실제 이산 확률분포
...
현재 유닛·무장 데이터는 `apps/web/src/data/catalog.ts`의 샘플 카탈로그로 제공됩니다. UI 컴포넌트와는 분리됐지만 아직 외부 JSON, 런타임 스키마 검증, 데이터 릴리스 관리, IndexedDB 및 특수 규칙은 후속 단계입니다. `D6 Test Blaster (Temporary)`는 가변 공격 계산을 확인하기 위한 임시 데이터이며 정식 게임 데이터가 아닙니다.

현재 `BattleInput.attacks`는 기존 숫자 입력과 `DiceExpression`을 모두 지원합니다. `BattleInput.damage`는 아직 고정 `number`이므로 가변 피해 주사위와 공격별 피해 할당 결합은 다음 단계입니다.
...
const bounds = diceExpressionBounds(attacksPerModel); // { minimum: 1, maximum: 6 }
const distribution = diceExpressionToPmf(attacksPerModel);
...
다음 작업은 `BattleInput.damage`에 `DiceExpression`을 연결하고, 실패한 내성마다 피해량 PMF를 적용해 모델별 피해 할당 상태를 합성하는 것입니다. 기존 고정 피해 입력과 non-spill 결과는 동일하게 유지해야 합니다.
