...
- 고정·D3·D6 조합의 대표 골든 테스트
- 확률 정규화, 결정성, 상태 유효성과 단조성 불변식 테스트
...
저장소를 처음 내려받거나 정확히 잠긴 의존성을 설치할 때는 루트에서 다음 명령을 사용합니다.
...
- CI와 GitHub Pages 배포는 `npm ci`만 사용합니다.
- 의존성을 변경할 때는 저장소 루트에서 `npm install` 또는 `npm install <package> --workspace <workspace>`를 실행합니다.
- `package.json` 변경과 함께 갱신된 `package-lock.json`을 같은 커밋에 포함합니다.
- `.npmrc`는 lockfile v3 생성과 정확한 버전 저장을 강제합니다.
...
계산 엔진 테스트에는 다음 파일이 포함됩니다.

```text
packages/calculator/src/index.test.ts          규칙 단위·회귀 테스트
...
다음 작업은 **정식 데이터 스키마를 정의하고 현재 TypeScript 샘플 카탈로그를 외부 JSON으로 분리하는 것**입니다. 이 단계에서 `Unit`과 `ModelProfile`의 책임을 분리하고, 중복 ID·누락 참조·잘못된 수치를 로딩 또는 빌드 단계에서 차단합니다.
